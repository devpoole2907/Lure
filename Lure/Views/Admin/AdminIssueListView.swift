...
struct AdminIssueListView: View {
    let apiClient: SeerrAPIClient

    @State private var viewModel: AdminIssueListViewModel
    @State private var errorAlert: ErrorAlertItem?

    init(apiClient: SeerrAPIClient) {
        self.apiClient = apiClient
        self._viewModel = State(initialValue: AdminIssueListViewModel(apiClient: apiClient))
    }
        ...
        List {
            Section {
                Picker("Status", selection: $viewModel.selectedFilter) {
                    ForEach(AdminIssueFilter.allCases) { filter in
            ...
            if viewModel.isLoading && viewModel.issues.isEmpty {
                ...
                Section {
                    ForEach(viewModel.issues) { issue in
                        NavigationLink {
                            AdminIssueDetailView(issue: issue, apiClient: apiClient) { updatedIssue in
                                viewModel.refreshIssue(updatedIssue)
                            }
                        } label: {
                            AdminIssueRow(issue: issue)
                        }
                    }
...
private struct AdminIssueSubtitleModifier: ViewModifier {
    let subtitle: String?
...
private struct AdminIssueRow: View {
    let issue: SeerrIssue

    var body: some View {
        HStack(spacing: 12) {
            PosterImage(
                url: issue.media?.posterURL,
                width: 44,
                height: 64,
                cornerRadius: LureDesign.CornerRadius.tinyPoster
            )
                ...
                HStack(spacing: 6) {
                    ...
                    if let status = issue.issueStatus {
                        Text(status.title)
                            ...
                            .padding(.vertical, 2)
                            .background(status == .resolved ? .green.opacity(LureDesign.Opacity.badgeBackground) : .orange.opacity(LureDesign.Opacity.badgeBackground), in: Capsule())
                            .foregroundStyle(status == .resolved ? .green : .orange)
                    }
                }
