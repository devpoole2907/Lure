...
struct AdminUserManagementView: View {
    ...
    init(apiClient: SeerrAPIClient) {
        ...
        self._viewModel = State(initialValue: AdminUserManagementViewModel(apiClient: apiClient))
    }
    ...
    var body: some View {
            ...
            if viewModel.isLoading && viewModel.users.isEmpty {
                ...
                Section {
                    ForEach(viewModel.users) { user in
                        NavigationLink {
                            AdminUserEditorView(user: user, apiClient: apiClient) { updatedUser in
        ...
        .modifier(AdminUserSubtitleModifier(subtitle: viewModel.users.isEmpty ? nil : "\(viewModel.totalUserCount) users"))
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 8) {
                    ...
                    Text(SeerrPermission.permissionLevelLabel(for: user.permissions))
                        ...
                        .padding(.vertical, 2)
                        .background(.blue.opacity(LureDesign.Opacity.badgeBackground), in: Capsule())
...
private extension AdminUserManagementView {
...
private struct AdminUserSubtitleModifier: ViewModifier {
    let subtitle: String?
